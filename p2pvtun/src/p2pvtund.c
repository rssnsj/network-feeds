...
static inline void p2pvtun_msg_set_uuids(struct p2pvtun_msg *nm,
		const char *src_uuid, const char *dst_uuid)
{
	memset(nm->hdr.src_uuid, 0x0, P2PCAT_UUID_SIZE);
	memset(nm->hdr.dst_uuid, 0x0, P2PCAT_UUID_SIZE);
	if (src_uuid)
		strncpy(nm->hdr.src_uuid, src_uuid, P2PCAT_UUID_SIZE);
	if (dst_uuid)
		strncpy(nm->hdr.dst_uuid, dst_uuid, P2PCAT_UUID_SIZE);
}
...
static void __cleanup_and_exit(int sig)
{
	if (g_sockfd >= 0 && is_valid_host_sin(&g_peer_addr)) {
		struct p2pvtun_msg nm;
		int i;

		memset(&nm, 0x0, sizeof(nm));
		nm.hdr.ver = 0;
		nm.hdr.opcode = P2PVTUN_MSG_DISCONNECT;
		p2pvtun_msg_set_uuids(&nm, g_uuid, g_expected_uuid);
		nm.disconnect.rsv = 0;
		for (i = 0; i < 2; i++) {
			sendto(g_sockfd, &nm, P2PVTUN_MSG_BASIC_HLEN + sizeof(nm.disconnect), 0,
...
#define NM_PI_BUFFER_SIZE  (2048)
	...
	struct p2pvtun_msg *nm = (void *)nm_buf;
...
#define is_in_p2pcat_mode() (0)
	...
	for (;;) {
		...
		if (current_ts - last_xmit_ts > g_keepalive_timeo) {
			...
			nm->hdr.ver = 0;
			nm->hdr.opcode = P2PVTUN_MSG_NOOP;
			p2pvtun_msg_set_uuids(nm, g_uuid, g_expected_uuid);
			nm->noop.rsv = 0;
			if (g_peer_addr.sin_addr.s_addr && g_peer_addr.sin_port) {
				sendto(g_sockfd, nm, P2PVTUN_MSG_BASIC_HLEN + sizeof(nm->noop), 0,
						(struct sockaddr *)&g_peer_addr, sizeof(g_peer_addr));
				last_xmit_ts = current_ts;
			}
		}
		...
		if (FD_ISSET(g_sockfd, &rset)) {
			...
			if (rc < 0)
				goto out1;
			if (rc < P2PVTUN_MSG_BASIC_HLEN)
				goto out1;

			/* Check identities of this packet. */
			if ((g_uuid && strncmp(nm->hdr.dst_uuid, g_uuid, P2PCAT_UUID_SIZE) != 0) ||
				(g_expected_uuid && strncmp(nm->hdr.src_uuid, g_expected_uuid, P2PCAT_UUID_SIZE) != 0)) {
			...
			case P2PVTUN_MSG_IPDATA:
				/* No packet is shorter than a 20-byte IPv4 header. */
				if (rc < P2PVTUN_MSG_IPDATA_OFFSET + 20)
					goto out1;
				ip_dlen = ntohs(nm->ipdata.ip_dlen);
				pi->flags = 0;
				pi->proto = nm->ipdata.proto;
				ready_dlen = (size_t)rc - P2PVTUN_MSG_IPDATA_OFFSET;
				if (g_crypto_passwd) {
...
out1:		;
		...
		if (FD_ISSET(g_tunfd, &rset)) {
			...
			case ETH_P_IPV6:
				...
				nm->hdr.opcode = P2PVTUN_MSG_IPDATA;
				p2pvtun_msg_set_uuids(nm, g_uuid, g_expected_uuid);
				...
				if (g_peer_addr.sin_addr.s_addr && g_peer_addr.sin_port) {
					rc = sendto(g_sockfd, nm_buf, P2PVTUN_MSG_IPDATA_OFFSET + ready_dlen, 0,
							(struct sockaddr *)&g_peer_addr, sizeof(g_peer_addr));
					last_xmit_ts = current_ts;
				}
				break;
